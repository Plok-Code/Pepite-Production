...
def kpi_2_writers(df_movies: pd.DataFrame) -> go.Figure:
    df = _dedupe_movies(df_movies)
    df = _explode_writers(df)
    if df.empty or "writer_name" not in df.columns:
        return _empty_figure("KPI 2 — Writers")

    work = df.copy()
    work["writer_name"] = work["writer_name"].fillna("").astype(str).str.strip()
    work = work[work["writer_name"] != ""]
    if work.empty:
        return _empty_figure("KPI 2 — Writers")

    for col in ("score_global", "movie_facebook_likes", "popularity"):
        if col not in work.columns:
            work[col] = np.nan

    writer_stats = (
        work.groupby("writer_name", as_index=False)
        .agg(
            nb_films=("writer_name", "size"),
    ...
    display = writer_stats[writer_stats["writer_name"].str.lower() != "unknown"]
    display_valid = display[display["nb_films"] >= 2]
    if display_valid.empty:
        return _empty_figure("KPI 2 — Writers")

    top_score_writer = display_valid.sort_values("score_moyen", ascending=False).iloc[0]
    score_moyen_global = float(display_valid["score_moyen"].mean())

    top_score_df = display_valid.sort_values("score_moyen", ascending=False).head(10)
    top_pop_df = display.sort_values("fb_likes_total", ascending=False).head(10)
    impact_df = display_valid.sort_values("nb_films", ascending=False).head(15)
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            "Writer référence",
            "Top 10 — Score moyen",
            "Top 10 — Popularité (Facebook likes films)",
    ...
    fig.add_trace(
        go.Indicator(
            mode="number+delta",
            value=float(top_score_writer["score_moyen"]),
            number={"font": {"size": 46, "color": _PRIMARY}, "suffix": " pts"},
            delta={
    ...
    fig.add_trace(
        go.Bar(
            x=top_pop_df["writer_name"],
            y=top_pop_df["fb_likes_total"],
            marker_color=_PRIMARY,
            text=top_pop_df["fb_likes_total"].round(0),
            textposition="outside",
            hovertemplate="<b>%{x}</b><br>FB likes films: %{y:,.0f}<br>Nb films: %{customdata}<extra></extra>",
    ...
    fig.add_trace(
        go.Scatter(
            ...
            y=impact_df["score_moyen"].round(2),
            mode="markers+text",
            text=impact_df["writer_name"],
            textposition="top center",
            marker=dict(
                size=np.clip(impact_df["fb_likes_total"].fillna(0).to_numpy() / 5000.0, 10, 50),
                color=_PRIMARY,
                opacity=0.8,
                line=dict(width=1, color=_GRID),
            ),
            hovertemplate=(
                "<b>%{text}</b><br>"
                "Nb films: %{x}<br>"
                "Score moyen: %{y:.2f}<extra></extra>"
            ),
        ),
        row=2,
        col=2,
    ...
    fig.update_xaxes(title_text="Nb films", row=2, col=2)
    fig.update_yaxes(title_text="Score moyen", row=2, col=2)
    fig.update_xaxes(tickangle=45, row=1, col=2)
    fig.update_xaxes(tickangle=45, row=2, col=1)
    return _apply_wildflix_layout(fig, "KPI 2 — Popularité & impact des writers", height=900)
